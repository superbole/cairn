...
| `PreToolUse` archive guard (v1.63.0+) | **An agent cannot archive its own session while the wrap verdict is `CAIRN OPEN`** — including commits made after a `SET`, which is how a wrapped session once ended unwrapped. It imports the same verdict `wrap_receipt.py --check` computes and refuses with the missing steps. `SET` and `NOT DUE` pass silently; `UNKNOWN` passes with one line, because it is not evidence anything was skipped; any error, another session's id, or a project with no `NEXT.md` passes. **Known limit:** a hook only sees tool calls — archiving from the sidebar fires nothing and `SessionEnd` cannot block, so this closes the agent's path only. The sidebar stays your way out, and the next session's "did not finish cleanly" box still catches it. |
...
|---|---|
| **Writes `~/.claude/CLAUDE.md`** | The plugin installs its rules into a marked block in your global, **always-loaded** instruction file — the text every Claude Code session on this machine reads before you type. It rewrites that block on every version change, **including an unattended marketplace auto-update you did not trigger**. Only the text between the markers is touched, anything you wrote outside them is left alone, and a backup is written first. **Since v1.63.0 the one-line notice says what moved** — lines added, removed and changed, the sections they sit in, and the exact `git diff` command against the backup — and **an older copy of the plugin never rolls a newer block back**: it writes nothing and tells you to update. **This is the largest thing you are trusting.** |
