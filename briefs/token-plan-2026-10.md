...
- **No Remote Control and no push notifications on the work account:** ICT blocks them. Don't plan
  ...
  GitHub's mobile app alerts him to a new PR. Whether work-GitLab merge requests can be read from France
  without the VPN is an open question. Ask it.
- **Review model: one branch and one PR (or MR) per item, and nothing merges to `main`
  unreviewed.** He wants the branches kept in order. Starting point, to confirm with him:
...
3. **Decide with him** (the checkpoint). Only what the section above leaves open:
   - whether DeepThought stays on during the trip, for the private account's local runs, or whether
     private work runs as cloud routines only;
   - when he can get to NB5 before Thursday to set it up;
   - whether work-GitLab MRs can be read from France without the VPN. If they can't, work-account
     GitLab output piles up until 20 Oct, and the plan should prefer GitHub-hosted work;
   - the branch naming scheme above: confirm it or change it.
